...
## Как это встраивается в «войну»

Сложный слой CUDA-compat живёт в тени: основной путь пользователя — открытые стандарты (Triton/SYCL/PyTorch). CUDA-compat нужен для **легаси-переноса**: существующие приложения запускаются у нас «как есть», пока команда мигрирует их на нативный путь. При этом юридическая рамка (ADR-0005): только открытые реализации, никакого копирования кода NVIDIA.
